...
def _apply_fields(df, fields_spec):
    ...
    if any("computed" in f for f in fields_spec):
        ...
        for field in fields_spec:
            ...
            # Optimization: list comprehension over records is ~3x faster than apply(axis=1)
...
def _handle_api(api_name, spec, resolved_data):
    ...
    if foreach:
        ...
        frames = []
        # Optimization: iterating over to_dict('records') is much faster than iterrows()
