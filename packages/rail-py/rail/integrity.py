...
def _normalize_legacy_claim_record(record: dict[str, Any]) -> dict[str, Any]:
    if "claim_key" in record and "claim_text" in record:
        ...
        if not isinstance(evidence_paths, list):
            evidence_paths = []
        discovered_in_paths = normalized.pop("discovered_in_paths", None)
        if isinstance(discovered_in_paths, list):
            evidence_paths = [
                *evidence_paths,
                *[str(item).strip() for item in discovered_in_paths if str(item).strip()],
            ]
        if evidence_paths:
            normalized["evidence_paths"] = list(dict.fromkeys(evidence_paths))
        caveats = normalized.get("caveats")
        if not isinstance(caveats, list):
            caveats = []
        scope_note = str(normalized.pop("scope_note", "") or "").strip()
        if scope_note:
            caveats.append(f"scope_note:{scope_note}")
        limitations = normalized.pop("limitations", None)
        if isinstance(limitations, list):
            caveats.extend(str(item).strip() for item in limitations if str(item).strip())
        snippet = str(normalized.pop("snippet", "") or "").strip()
        if snippet:
            caveats.append(f"legacy_snippet:{snippet}")
        if caveats:
...
def _normalize_legacy_claim_candidate_record(record: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "snippet": str(record.get("snippet") or "").strip() or None,
        "source_path": str(record.get("source_path") or _default_source_path(STATE_FILE_NAMES["claim_candidates"])),
