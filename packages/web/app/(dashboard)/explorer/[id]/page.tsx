  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Properties */}
        <div>
          <h2 className="text-sm font-semibold text-[--muted-foreground] uppercase tracking-wide mb-3">Properties</h2>
                ...
                <tbody>
                  {propEntries.map(([key, val]) => (
                    <tr key={key} className="border-b border-[--border] last:border-0">
                      <td className="px-4 py-2.5 text-[--muted-foreground] font-mono text-xs w-40 shrink-0">{key}</td>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[--border] bg-[--muted]">
                      <th className="text-left px-4 py-2 text-[--muted-foreground] font-medium text-xs">Property</th>
                      <th className="text-left px-4 py-2 text-[--muted-foreground] font-medium text-xs">Target</th>
                    </tr>
                  </thead>
                  <tbody>
                    {entity.relationships.map((rel, i) => (
                      <tr key={i} className="border-b border-[--border] last:border-0 hover:bg-white/[0.03]">
                        ...
                        <td className="px-4 py-2.5">
                          ...
                          <span className="text-[--muted-foreground] font-mono text-xs ml-2">{rel.targetId}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {/* 1-hop graph */}
        <div>
          <h2 className="text-sm font-semibold text-[--muted-foreground] uppercase tracking-wide mb-3">Graph</h2>
          <div
            ref={containerRef}
          ...
          >
            {graph && FG ? (
              ...
              <FG
                ...
                nodeVal={(n: { id: string }) => n.id === decoded ? 10 : 4}
