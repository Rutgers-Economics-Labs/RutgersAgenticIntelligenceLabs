    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <span
              ...
              style={{
                ...
                background: `${STATUS_COLORS[jobData.status]}22`,
                border: `1px solid ${STATUS_COLORS[jobData.status]}44`,
              }}
            >
              {jobData.status}
            </span>
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-[--muted-foreground]">
            <span>Started: {formatDate(jobData.startedAt ?? jobData.createdAt)}</span>
            {jobData.finishedAt && <span>Finished: {formatDate(jobData.finishedAt)}</span>}
            <span>Duration: {formatDuration(jobData.startedAt ?? jobData.createdAt, jobData.finishedAt)}</span>
          </div>
        </div>
      ...
      {jobData.status === "failed" && jobData.errorMessage && (
        <div className="flex items-start gap-3 rounded-xl border border-red-700 bg-red-900/20 p-4 text-sm text-red-300">
          ...
          <div>
            ...
            <p className="mt-1 whitespace-pre-wrap">{jobData.errorMessage}</p>
          </div>
        </div>
      )}
      ...
      {isExecution && execution && (
        <section className="space-y-3">
          ...
          <pre className="overflow-x-auto rounded-xl border border-[--border] bg-[--muted]/30 p-4 font-mono text-xs text-[--foreground]">
            {execution.input}
          </pre>
        </section>
      )}
      ...
      <section className="space-y-3">
        ...
        <div
          ...
          className="max-h-[480px] overflow-y-auto rounded-xl border border-[--border] bg-[--muted]/30 p-3 font-mono text-xs"
        >
          {sortedLogs.length === 0 && (
          ...
          <div className="space-y-1">
            {sortedLogs.map((log) => {
              const color =
                log.level === "error" || log.level === "stderr" ? "text-red-500 dark:text-red-400" :
                log.level === "warn" ? "text-amber-600 dark:text-amber-400" :
                log.level === "stdout" ? "text-blue-600 dark:text-blue-300" :
                "text-[--foreground]";
