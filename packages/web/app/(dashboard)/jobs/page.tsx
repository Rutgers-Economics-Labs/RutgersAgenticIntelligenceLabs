...
function JobsPageInner() {
  ...
  const hydrationJobs = useQuery(
    projectId ? api.jobs.listByProject : api.jobs.list,
    projectId ? { projectId, limit: 100 } : { limit: 100 }
  );
  
  const executionJobs = useQuery(
    projectId ? (api as any).executions.listByProject : (api as any).executions.list,
    projectId ? { projectId, limit: 100 } : { limit: 100 }
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[--muted-foreground] mt-1">
            {projectId ? "Isolated view of this project's executions." : "All recent activity across the platform."}
          </p>
        </div>
                    ...
                    <div className="flex flex-col">
                      ...
                      {!projectId && job.projectId && (
                        <span className="text-[10px] text-[--primary] mt-1 flex items-center gap-1">
                           <Zap size={10} /> ID: {job.projectId.slice(-8)}
                        </span>
                      )}
                    </div>
                  </td>
