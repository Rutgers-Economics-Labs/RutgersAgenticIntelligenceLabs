...
type QualityReport = {
  projectSlug?: string; generatedAt: number;
...
function QualityPageInner({ projectSlug }: { projectSlug: string }) {



  const snapshots = useQuery(api.quality.listSnapshots, { projectSlug: projectSlug as any, limit: 10 });
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      const [r, d] = await Promise.all([
        qualityApi.report(projectSlug),
        qualityApi.diff(projectSlug),
      ]);
      setReport(r);
      setDiff(d);
    } catch (e: any) {
  ...
  const takeSnapshot = async () => {
    ...
    try {
      ...
      await qualityApi.snapshot(projectSlug, label);
      const d = await qualityApi.diff(projectSlug);
      setDiff(d);
    } catch (e: any) {
      setError(e.message);
    } finally {
