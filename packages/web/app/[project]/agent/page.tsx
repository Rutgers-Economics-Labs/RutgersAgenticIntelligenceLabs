...
// ToolResult removed — now using shared component from @/components/jobs/ToolResult
...
export default async function WorkspacePage({ params }: { params: Promise<{ project: string }> }) {
  return (
    <Suspense fallback={<div className="flex h-screen items-center justify-center text-sm text-[--muted-foreground]">Loading workspace…</div>}>
      <WorkspacePageInner projectSlug={(await params).project} />
    </Suspense>
  );
}
