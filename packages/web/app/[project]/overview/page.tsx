...
function OverviewContent({ projectSlug }: { projectSlug: string }) {
  ...
  const { data: classesData } = useSWR(
    `/api/v1/ontology/classes?project=${projectSlug}`,
    fetcher,
  );
  const classesCount = classesData?.classes?.length || 0;

  const { data: sqlData } = useSWR(
    `/api/v1/sql?project=${projectSlug}&query=${encodeURIComponent("SELECT SUM(estimated_size) as count FROM duckdb_tables")}`,
    fetcher,
  );
    ...
    <div className="flex flex-col gap-6 max-w-6xl mx-auto w-full pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {project.lastHydratedAt
                ? formatDistanceToNow(project.lastHydratedAt, {
                    addSuffix: true,
                  })
                : "Never"}
            </div>
