...
async def test_record_claim_rejects_unknown_source_keys(client, convex_mock, tmp_path):
    ...
    resp = await client.post(
        ...
        json={
            "claimKey": "claim-001",
            "statement": "This claim points at a missing source.",
            "artifactPath": "artifacts/report.md",
            "status": "supported",
...
async def test_record_claim_rejects_unknown_evidence_chunk_keys(client, convex_mock, tmp_path):
    ...
    resp = await client.post(
        ...
        json={
            "claimKey": "claim-001",
            "statement": "This claim points at a missing chunk.",
            "artifactPath": "artifacts/report.md",
            "status": "supported",
            "sourceKeys": ["briefing-note"],
            "evidenceChunkKeys": ["missing-chunk"],
            "evidenceKind": "direct",
        },
    )
    ...
    assert resp.status_code == 422
    payload = resp.json()
    assert "Claim references unknown evidence chunk keys: missing-chunk" in payload["detail"]
...
async def test_record_claim_rejects_missing_evidence_paths(client, convex_mock, tmp_path):
    ...
    resp = await client.post(
        ...
        json={
            "claimKey": "claim-001",
            "statement": "This claim points at a missing evidence file.",
            "artifactPath": "artifacts/report.md",
            "status": "supported",
            "sourceKeys": ["briefing-note"],
            "evidencePaths": ["topics/missing-evidence.md"],
            "evidenceKind": "direct",
        },
    )
    ...
    assert resp.status_code == 422
    payload = resp.json()
    assert "Claim references missing evidence paths: topics/missing-evidence.md" in payload["detail"]
...
async def test_record_source_rejects_unknown_admissibility_status(client, convex_mock, tmp_path):
    ...
    resp = await client.post(
        ...
        json={
            ...
            "admissibilityStatus": "hearsay",
        },
    )

    assert resp.status_code == 422
    payload = resp.json()
    assert "Source admissibility must be one of" in payload["detail"]
...
async def test_record_source_rejects_unknown_freshness_status(client, convex_mock, tmp_path):
    ...
    assert resp.status_code == 422
    payload = resp.json()
    assert "Source freshness must be one of" in payload["detail"]
...
async def test_record_source_rejects_unknown_quality_status(client, convex_mock, tmp_path):
    ...
    resp = await client.post(
        ...
        json={
            ...
            "sourceType": "dataset",
            "title": "BLS LAUS",
            "qualityStatus": "trusted-ish",
        },
    )

    assert resp.status_code == 422
    payload = resp.json()
    assert "Source quality must be one of" in payload["detail"]
...
async def test_record_source_rejects_unknown_impact_level(client, convex_mock, tmp_path):
    ...
    assert resp.status_code == 422
    payload = resp.json()
    assert "Source impact level must be one of" in payload["detail"]
...
async def test_record_source_rejects_validated_derived_status_without_derivation_lineage(client, convex_mock, tmp_path):
    ...
    resp = await client.post(
        "/api/v1/projects/integrity-router-project/integrity/sources",
        json={
            "sourceKey": "derived-series",
            "sourceType": "dataset",
            "title": "Derived Series",
            "url": "https://example.com/derived.csv",
            "publisher": "Internal",
            "accessDate": "2026-05-14T00:00:00Z",
            "accessMethod": "manual",
            "freshnessStatus": "fresh",
            "qualityStatus": "validated",
            "admissibilityStatus": "derived",
    ...
    assert resp.status_code == 422
    payload = resp.json()
    assert "Validated derived sources must declare `derived_from` provenance" in payload["detail"]
...
async def test_artifact_lineage_write_rejects_unknown_claim_references(client, convex_mock, tmp_path):
    ...
    payload = artifact_resp.json()
    assert "Artifact lineage references unknown claim keys: missing-claim" in payload["detail"]
...
async def test_artifact_lineage_write_rejects_unknown_verification_runs(client, convex_mock, tmp_path):
    ...
    artifact_resp = await client.post(
        ...
        json={
            ...
            "title": "Report",
            "promotionState": "draft",
            "verificationRuns": ["research_plan/state/verification_runs.json#missing-run"],
        },
    )
...
async def test_patch_source_rejects_unknown_admissibility_status(client, convex_mock, tmp_path):
    ...
    resp = await client.patch(
        "/api/v1/projects/integrity-router-project/integrity/sources/bls-laus",
        json={"admissibilityStatus": "hearsay"},
    )
...
async def test_patch_source_rejects_unknown_quality_status(client, convex_mock, tmp_path):
    ...
    resp = await client.patch(
        "/api/v1/projects/integrity-router-project/integrity/sources/bls-laus",
        json={"qualityStatus": "trusted-ish"},
    )
...
async def test_patch_source_rejects_validated_status_without_provenance(client, convex_mock, tmp_path):
    ...
    resp = await client.patch(
        "/api/v1/projects/integrity-router-project/integrity/sources/bls-laus",
        json={"qualityStatus": "validated", "admissibilityStatus": "observed"},
    )
...
async def test_patch_source_rejects_validated_derived_status_without_derivation_lineage(client, convex_mock, tmp_path):
    ...
    resp = await client.patch(
        "/api/v1/projects/integrity-router-project/integrity/sources/derived-series",
        json={
            "qualityStatus": "validated",
            "admissibilityStatus": "derived",
            "freshnessStatus": "fresh",
