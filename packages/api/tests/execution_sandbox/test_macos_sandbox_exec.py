...
from app.execution.sandbox import MacOSSandboxExec, SandboxUnavailableError, detect_capability, select_sandbox
...
def test_allowed_write_succeeds_and_environment_is_filtered(sandbox: MacOSSandboxExec, tmp_path: Path) -> None:
    result = _run(sandbox, tmp_path, "import os, pathlib; pathlib.Path('allowed.txt').write_text(os.environ['SAFE']); print(os.getenv('SECRET'))")
...
def test_timeout_and_cancellation_clean_up_the_sandbox_process_group(sandbox: MacOSSandboxExec, tmp_path: Path) -> None:
