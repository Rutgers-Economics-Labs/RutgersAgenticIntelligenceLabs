...
from app.execution.models import TERMINAL_STATUSES
...
RunId = Annotated[str, Path(min_length=1, max_length=128, pattern=r"^[A-Za-z0-9][A-Za-z0-9._-]*$")]
...
class EventCursorError(PlatformError):
    status_code = 409
    code = "event_cursor_unknown"
...
def create_workflow_run(project_id: ProjectId, payload: CreateWorkflowRunRequest,
    ...
    if project.access.value == "read_only":
...
def list_runs(project_id: ProjectId, registry: ProjectRegistry = Depends(get_registry), service: ExecutionService = Depends(get_execution)) -> RunListResponse:
    ...
    records = sorted((r for r in service.store.records() if r.project_id == project_id), key=lambda r: r.created_at, reverse=True)
    return RunListResponse(runs=[RunDTO.from_record(r) for r in records])


@router.get("/{run_id}", response_model=RunResponse)
def get_run(project_id: ProjectId, run_id: RunId, service: ExecutionService = Depends(get_execution)) -> RunResponse:
    return RunResponse(run=RunDTO.from_record(_run_for_project(service, project_id, run_id)))


@router.post("/{run_id}/cancel", response_model=RunResponse)
def cancel_run(project_id: ProjectId, run_id: RunId, service: ExecutionService = Depends(get_execution)) -> RunResponse:
    _run_for_project(service, project_id, run_id)
    return RunResponse(run=RunDTO.from_record(service.cancel(run_id)))


@router.get("/{run_id}/events", response_model=RunEventsResponse)
def event_snapshot(project_id: ProjectId, run_id: RunId, service: ExecutionService = Depends(get_execution)) -> RunEventsResponse:
    _run_for_project(service, project_id, run_id)
    return RunEventsResponse(events=[RunEventDTO.from_event(event) for event in service.events(run_id)])


@router.get("/{run_id}/events/stream")
async def event_stream(project_id: ProjectId, run_id: RunId, request: Request,
                       last_event_id: Annotated[str | None, Header()] = None,
                       service: ExecutionService = Depends(get_execution)) -> StreamingResponse:
    _run_for_project(service, project_id, run_id)
    events_at_open = service.events(run_id)
    if last_event_id:
        found = [i for i, event in enumerate(events_at_open) if event.event_id == last_event_id]
        if not found:
            raise EventCursorError("Event cursor is no longer available")
        initial_cursor = found[-1] + 1
    else:
        initial_cursor = 0
    async def generate():
