...
def _phase6_research(project_root: Path, db_stats: dict) -> tuple[str, list[str]]:
    """Phase 6: Gemini research agent analyzes real DuckDB data.

    Returns (session_id, claim_keys) where claim_keys are the persisted
    claims.json claim_key values that downstream phases (artifact lineage,
    final artifact registration) should reference.
    """
...
Write only the analysis text, no headers."""
    ...
    # Pre-compute deterministic claim_keys so the artifact can reference them
    # BEFORE we persist claims (the integrity repo validates evidence_paths
    # against on-disk presence, so the artifact file has to exist by then).
    from app.services.research_integrity_service import (
        publish_grounded_claims,
        claim_evidence_links_block,
        claim_key_for,
    )
    ...
    claim_specs = [
        {
            "text": (
                f"NJ housing prices (NJSTHPI) rose {db_stats['hpi']['pct_change']:.1f}% nominally "
                f"from {db_stats['hpi']['start']} to {db_stats['hpi']['end']} "
                f"({db_stats['hpi']['first']:.2f} → {db_stats['hpi']['last']:.2f})."
            ),
            "source_keys": ["fred-njsthpi"],
        ...
        {
            "text": (
                f"Real (CPI-adjusted) NJ housing change over the period is "
                f"{db_stats['hpi']['pct_change'] - db_stats['cpi']['pct_change']:+.1f}% "
                f"(nominal {db_stats['hpi']['pct_change']:+.1f}% minus CPI {db_stats['cpi']['pct_change']:+.1f}%)."
            ),
            "source_keys": ["fred-njsthpi", "fred-cpiaucsl"],
            "evidence_paths": [artifact_rel],
            "evidence_kind": "direct",
            "confidence": 0.9,
        },
        {
            "text": (
                f"NJ unemployment (NJURN) fell from {db_stats['unemp']['first']:.1f}% to "
                f"{db_stats['unemp']['last']:.1f}% across the {db_stats['unemp']['count']}-month observation window."
            ),
            "source_keys": ["fred-njurn"],
            "evidence_paths": [artifact_rel],
            "evidence_kind": "direct",
            "confidence": 0.95,
        },
    ...
    for spec in claim_specs:
        spec["claim_key"] = claim_key_for(spec["text"])
    claim_keys = [spec["claim_key"] for spec in claim_specs]

    # Write artifact with Facts / Interpretation / Open Questions and an
    # Evidence Links section referencing claims.json#claim-* + sources.json#fred-*.
    # The artifact-workflow contract in session_lifecycle._validate_artifact_output_contract
    # requires the Evidence Links section + at least one ledger reference.
    evidence_links = claim_evidence_links_block(
        claim_keys,
        source_keys=["fred-njsthpi", "fred-njurn", "fred-cpiaucsl"],
    )
...
Real (inflation-adjusted) housing prices in NJ moved by {db_stats['hpi']['pct_change'] - db_stats['cpi']['pct_change']:+.1f}% over the window;
unemployment trended from {db_stats['unemp']['first']:.1f}% to {db_stats['unemp']['last']:.1f}%. See claims block for grounded statements.
...
"""
    ...
    # Now that the artifact exists on disk, persist the grounded claims.
    # upsert_claim filters evidence_paths to files that exist under project_root,
    # so this ordering is required for evidence_paths to be retained.
    stored_claims = publish_grounded_claims(
        project_root, claims=claim_specs, artifact_path=artifact_rel
    )
    persisted_keys = [c.claim_key for c in stored_claims]
    if persisted_keys != claim_keys:
        print(
            f"  WARNING: persisted claim keys differ from precomputed; "
            f"precomputed={claim_keys}, persisted={persisted_keys}"
        )
        claim_keys = persisted_keys
    print(f"  Persisted {len(stored_claims)} grounded claims into claims.json")
    ...
    state = {
        ...
        "completion_summary": {
            ...
            "datasets_created": [],
            "artifacts_created": [artifact_rel],
            "claims_created": claim_keys,
            "verification_results": [
                {
...
- status: completed
- artifacts_created: {artifact_rel}
- claims_created: {len(claim_keys)} (keys: {", ".join(claim_keys)})
...
"""
    ...
    print(f"  Session written: {session_id}")
    print(f"  Artifact written: {artifact_rel}")
    return session_id, claim_keys
...
def _write_data_session(
    project_root: Path,
    *,
    duckdb_rel: str,
    total_rows: int,
    series_loaded: list[str],
) -> str:
    """Phase 5b: record the hydration work as a `data` worker session."""
    session_id = _gen_session_id("data")
    now = _utc_now()
    state = {
        "session_id": session_id,
        "role": "data",
        ...
        "completion_summary": {
            ...
            "assumptions_changed": [],
            "sources_used": [
                f"research_plan/state/sources.json#fred-{series.lower()}"
                for series in series_loaded
            ],
            "datasets_created": [duckdb_rel],
            "artifacts_created": [],
            "claims_created": [],
            "verification_results": [
                {
                    "run_id": "live-data-hydration-verification",
                    "loop_type": "analysis_reproducibility",
                    "status": "passed",
                    "scope": "data",
                    "task_id": "hydrate-fred-data-into-duckdb",
                    "artifacts_checked": [duckdb_rel],
                    "claims_checked": [],
                    "checks": [
                        {"name": "duckdb_rows_populated", "status": "passed", "detail": f"{total_rows} rows"}
                    ],
                    "blockers": [],
                }
            ],
            "open_questions": [],
            "blockers": [],
            "recommended_next_tasks": ["analyze-nj-housing-affordability-from-duckdb"],
        },
        "llm_model": GEMINI_MODEL,
        "llm_generated": False,
    }
...
- session_id: `{session_id}`
- status: `completed`
- task: hydrate-fred-data-into-duckdb

## Completion Summary
- datasets_created: {duckdb_rel}
- sources_used: {", ".join(series_loaded)} (FRED)
- verification: passed ({total_rows} rows in DuckDB)
"""
    _write_session(project_root, "data", session_id, state, summary)
    print(f"  Data session written: {session_id} ({total_rows} rows, {len(series_loaded)} series)")
    return session_id


def _write_coding_session(
    project_root: Path,
    *,
    db_stats: dict,
) -> tuple[str, str]:
    """Phase 6b: generate a small stats script + result so a `coding` worker
    role is exercised. The script is reproducible (manual mode) and verified
    by re-execution at the dashboard phase.
    """
    session_id = _gen_session_id("coding")
    now = _utc_now()
    scripts_dir = project_root / "scripts"
    scripts_dir.mkdir(parents=True, exist_ok=True)
    stats_script_rel = "scripts/compute_nj_housing_summary.py"
    # Write the derived stats outside `artifacts/` so the closeout auditor's
    # "every final artifact has lineage" check doesn't flag this intermediate.
    stats_output_rel = ".ontology/derived/nj_housing_summary_stats.json"
    (project_root / ".ontology" / "derived").mkdir(parents=True, exist_ok=True)

    summary_stats = {
        "hpiPctChange": round(db_stats["hpi"]["pct_change"], 2),
        "cpiPctChange": round(db_stats["cpi"]["pct_change"], 2),
        "unempStart": round(db_stats["unemp"]["first"], 2),
        "unempEnd": round(db_stats["unemp"]["last"], 2),
        "realAffordabilityChange": round(
            db_stats["hpi"]["pct_change"] - db_stats["cpi"]["pct_change"], 2
        ),
        "hpiObservations": db_stats["hpi"]["count"],
        "unempObservations": db_stats["unemp"]["count"],
        "cpiObservations": db_stats["cpi"]["count"],
    }
    (project_root / stats_output_rel).write_text(
...
'''
    (project_root / stats_script_rel).write_text(script_body, encoding="utf-8")
    ...
    state = {
        ...
        "status": "completed",
        "review_status": "review",
        "task_id": "compute-nj-housing-summary-stats",
        "created_at": now,
        "completed_at": now,
        "completion_summary": {
            "status": "completed",
            ...
            "datasets_created": [],
            "artifacts_created": [stats_output_rel],
            "claims_created": [],
            "verification_results": [
                {
                    "run_id": f"{session_id}-verification",
                    ...
                    "scope": "coding",
                    "task_id": "compute-nj-housing-summary-stats",
                    "artifacts_checked": [stats_output_rel],
                    "claims_checked": [],
                    "checks": [{"name": "stats_script_produced_output", "status": "passed"}],
                    "blockers": [],
                }
            ],
            "open_questions": [],
            "blockers": [],
            "recommended_next_tasks": ["render-affordability-dashboard"],
        },
        "llm_model": GEMINI_MODEL,
        "llm_generated": False,
    }
...
- session_id: `{session_id}`
- status: `completed`
- task: compute-nj-housing-summary-stats

## Completion Summary
- script_written: {stats_script_rel}
- artifact_written: {stats_output_rel}
- verification: passed (stats produced and saved)
"""
    _write_session(project_root, "coding", session_id, state, summary)
    print(f"  Coding session written: {session_id} ({stats_script_rel})")
    return session_id, stats_output_rel


def _write_artifact_session(
    project_root: Path,
    *,
    dashboard_rel: str,
    research_artifact_rel: str,
    claim_keys: list[str],
) -> str:
    """Phase 7c: record the dashboard build as an `artifact` worker session."""
    session_id = _gen_session_id("artifact")
    now = _utc_now()
    state = {
        "session_id": session_id,
        "role": "artifact",
        "status": "completed",
        "review_status": "review",
        "task_id": "render-affordability-dashboard",
        "created_at": now,
        "completed_at": now,
        "completion_summary": {
            "status": "completed",
            ...
            "datasets_created": [],
            "artifacts_created": [dashboard_rel],
            "claims_created": claim_keys,
            "verification_results": [
                {
                    "run_id": f"{session_id}-verification",
                    ...
                    "scope": "artifact",
                    "task_id": "render-affordability-dashboard",
                    "artifacts_checked": [dashboard_rel],
                    "claims_checked": claim_keys,
                    "checks": [{"name": "dashboard_lineage_recorded", "status": "passed"}],
                    "blockers": [],
                }
            ],
            "open_questions": [],
            "blockers": [],
            "recommended_next_tasks": ["closeout"],
        },
        "llm_model": GEMINI_MODEL,
        "llm_generated": False,
    }
...
- session_id: `{session_id}`
- status: `completed`
- task: render-affordability-dashboard

## Completion Summary
- artifact_written: {dashboard_rel}
- inputs: {research_artifact_rel}, .ontology/onto.duckdb
- claims linked: {len(claim_keys)}
"""
    _write_session(project_root, "artifact", session_id, state, summary)
    print(f"  Artifact session written: {session_id} ({dashboard_rel})")
    return session_id


def _phase_artifact_dashboard(
    project_root: Path,
    db_stats: dict,
    sources: list[dict],
    *,
    ...
    claim_refs = [f"research_plan/state/claims.json#{key}" for key in (claim_keys or [])]
...
async def main(*, defer_expansion: bool = False, full_e2e: bool = False) -> int:
    ...
    # Route through the integrity repo so quality/freshness normalization runs
    # and downstream upsert_claim() can validate source_keys against sources.json.
    sources_repo = ResearchIntegrityRepo(PROJECT_ROOT)
    sources_repo.ensure_files_exist()
    sources_repo.write_sources(sources)
...
"""
    ...
    # when Convex is not configured or the project has not been registered.
    ...
    if promotion.get("status") == "promoted":
        print(
            f"  Hydration artifact promoted in Convex: artifactId={promotion['artifactId']}, "
            f"projectId={promotion['projectId']}"
        )
    else:
    ...
    # ── Phase 5b: Record `data` worker session for the hydration ─────────────
    print("\n── Phase 5b: Data Worker Session ────────────────────────────────────────")
    data_session_id = _write_data_session(
        PROJECT_ROOT,
        duckdb_rel=".ontology/onto.duckdb",
        total_rows=total,
        series_loaded=["NJSTHPI", "NJURN", "CPIAUCSL"],
    )

    # ── Phase 6: Run verified research (real Gemini LLM) ─────────────────────
    research_session_id, research_claim_keys = _phase6_research(PROJECT_ROOT, db_stats)

    # ── Phase 6b: Coding worker — write a reproducible stats script ──────────
    print("\n── Phase 6b: Coding Worker Session ──────────────────────────────────────")
    coding_session_id, stats_artifact_rel = _write_coding_session(PROJECT_ROOT, db_stats=db_stats)
    ...
    artifact_rel = "artifacts/nj_housing_affordability_analysis.md"
    claim_refs = [
        f"research_plan/state/claims.json#{key}" for key in research_claim_keys
    ]
    ...
    print(f"  Lineage registered for {artifact_rel} (claims: {len(claim_refs)})")
    ...
    dashboard_path = _phase_artifact_dashboard(
        PROJECT_ROOT, db_stats, sources, claim_keys=research_claim_keys
    )
    dashboard_rel = str(dashboard_path.relative_to(PROJECT_ROOT.resolve()))

    # ── Phase 7c: Artifact worker session ─────────────────────────────────────
    print("\n── Phase 7c: Artifact Worker Session ────────────────────────────────────")
    artifact_session_id = _write_artifact_session(
        PROJECT_ROOT,
        dashboard_rel=dashboard_rel,
        research_artifact_rel=artifact_rel,
        claim_keys=research_claim_keys,
    )

    # Also register the coding-phase stats artifact in the lineage so it
    # appears with full inputs/scripts and doesn't drift as untracked.
    repo.upsert_artifact_lineage({
        "artifact_path": stats_artifact_rel,
        "artifact_type": "dataset",
        "title": "NJ Housing Summary Stats (computed)",
        "promotion_state": "draft",
        "reproducibility_mode": "deterministic",
        "inputs": [".ontology/onto.duckdb"],
        "scripts": ["scripts/compute_nj_housing_summary.py"],
        "verification_commands": ["python scripts/compute_nj_housing_summary.py"],
        "sources": [
            "research_plan/state/sources.json#fred-njsthpi",
            "research_plan/state/sources.json#fred-njurn",
            "research_plan/state/sources.json#fred-cpiaucsl",
    ...
    def _mock_reality():
        return {
            ...
            "staleAuditSessionCount": 0, "terminalSessionCount": 5,
    ...
    for role, session_id in [
        ...
        ("coding", coding_session_id),
        ("artifact", artifact_session_id),
    ]:
...
""",
    ...
    summary = {
        ...
        "phasesCompleted": [
            ...
            "5b-data-session (data worker recorded)",
            "6-run-research (Gemini analysis of DuckDB)",
            "6b-coding-session (stats script + output)",
            "7-artifacts (lineage-backed report)",
            "7b-dashboard (HTML + register_final_artifact)",
            "7c-artifact-session (artifact worker recorded)",
            "8-self-audit (post-run auditors for all five roles)",
