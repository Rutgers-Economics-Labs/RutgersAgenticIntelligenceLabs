...
def add_state_abbreviations(df, **kwargs):
    """
    Add an 'abbr' column (e.g. 'NJ') from the 'fips' column.
    Rows with no known abbreviation are dropped.
    """
    if "fips" not in df.columns:
        return df
    df = df.copy()
    df["abbr"] = df["fips"].astype(str).map(_FIPS_TO_ABBR)
    return df.dropna(subset=["abbr"]).reset_index(drop=True)
