...
type Props = { projects: Resource<Project[]>; selectedProjectId?: string; search: Search; find?: Resource<FindResponse>; graph?: Resource<GraphResponse>; sources?: Resource<SourcesResponse>; integrity?: Resource<IntegrityResponse>; affected?: Resource<SourceImpactResponse>; workflows?: Resource<WorkflowInventoryResponse>; approvals?: Resource<ApprovalInventoryResponse> };
...
function Provenance({ record, onClose }: { record: RecordRef; onClose: () => void }) {
  return <aside className={styles.drawer} aria-label="Record provenance"><div className={styles.drawerTop}><div><span className={styles.eyebrow}>Live platform data</span><h2>{record.label}</h2></div><button type="button" onClick={onClose} aria-label="Close provenance">×</button></div><dl><div><dt>KRAIL ID</dt><dd>{record.id}</dd></div><div><dt>Kind</dt><dd>{record.kind}</dd></div>{record.path && <div><dt>Project path</dt><dd>{record.path}</dd></div>}</dl>{record.metadata && Object.keys(record.metadata).length > 0 && <pre>{JSON.stringify(record.metadata, null, 2)}</pre>}</aside>;
}
...
function Graph({ resource, open }: { resource?: Resource<GraphResponse>; open: (record: RecordRef) => void }) {
  ...
  const nodes = [...graph.nodes, ...graph.documents.map((document) => ({ id: document.id, label: document.title, kind: document.kind, metadata: { path: document.path, topics: document.topics, entities: document.entities } }))].slice(0, 12);
  ...
  return <section className={styles.graphPanel} aria-labelledby="graph-title"><div className={styles.sectionTop}><div><span className={styles.eyebrow}>Connected records</span><h2 id="graph-title">Entity, document & source graph</h2></div><span className={styles.count}>{graph.nodes.length + graph.documents.length} records · {graph.edges.length} links</span></div>
...
export function KrailExplore({ projects, selectedProjectId, search, find, graph, sources, integrity, affected, workflows, approvals }: Props) {
